...
export default function Edit({ order, clients, products }: Props) {
    const [items, setItems] = useState<OrderItem[]>(() => {
        if (!order.items || order.items.length === 0) {
            return [{ product_id: 0, qty: 1, discount: 0, discount_type: 'percentage' }];
        }
        return order.items.map((item) => ({
            product_id: item.product_id,
            qty: item.qty,
            discount: item.discount || 0,
            discount_type: (item.discount_type === 'value' ? 'value' : 'percentage') as 'percentage' | 'value',
        }));
    });

    const [clientSearch, setClientSearch] = useState(() => {
        const currentClient = clients.find(c => c.id === order.client_id);
        return currentClient?.client_name || '';
    });
    const [showClientDropdown, setShowClientDropdown] = useState(false);
    const [clientList, setClientList] = useState(clients);
    const clientDropdownRef = useRef<HTMLDivElement>(null);

    const parseDeliveryDateTime = (dateStr: string | null) => {
    ...
    const { date: initialDate, time: initialTime } = parseDeliveryDateTime(order.delivery_date);

    const { data, setData, put, processing, errors } = useForm({
        client_id: order.client_id.toString(),
        items: [] as OrderItem[],
        is_vat: order.is_vat || false,
        delivery_date: initialDate,
        delivery_time: initialTime,
    });
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <Input
                                        ...
                                        min={order.order_created_at ? new Date(order.order_created_at).toISOString().split('T')[0] : ''}
                                        required
                                    />
